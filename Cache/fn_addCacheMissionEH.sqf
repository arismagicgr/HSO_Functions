...
private _alreadyExists = !isNil { missionNamespace getVariable ["HSO_cacheMissionEHID", nil] };

// If it already exists, exit the function
if (_alreadyExists) exitWith {diag_log "HSO Cache: Cache Mission EH already exists. No EH was added"; };

// Add the mission EH and get the ID for deletion by the HSO_fnc_disableCache
private _missionEHID = addMissionEventHandler ["GroupCreated", {
  ...
  private _isNil = isNil {_grp getVariable ["HSO_cacheEHID", nil]};
  ...
  // Store the EH ID in group's namespace to be available for deletion later by the HSO_fnc_disableCache
  _grp setVariable ["HSO_cacheEHID", _id, true];
}];

// Store the mission EH ID for deletion later by the HSO_fnc_disableCache
missionNamespace setVariable ["HSO_cacheMissionEHID", [_missionEHID, "GroupCreated"], true];
